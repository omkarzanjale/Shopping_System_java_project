...
<body style="text-align: center;">
    ...
    <%
        ...
        try{
            ...
            String sql = "SELECT * FROM product WHERE prid IN(SELECT prid FROM product_category WHERE cgid =(SELECT cgid FROM category WHERE cgname=?))";
            pst  = con.prepareStatement(sql); 
            pst.setString(1,cgname);
            rs = pst.executeQuery();
            while(rs.next()){
                if(rs.getInt(7)>0){%>
                    <div class = "name">
                ...
                <%}else{ %>
                    <div class = "name">
                        <form action="Advancebook.jsp" method="post">
                            <h2>Out of stock <br>Book In Advance(BIA)</h2><br>
                            <h3>Product Name : <%= rs.getString(2)%></h3><br>
                            <h4>Brand : <%= rs.getString(3)%></h4><br>
                            <h5><%= rs.getString(4)%></h5><BR>
